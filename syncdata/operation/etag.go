...
func etagV1WithoutBase64Encoded(reader io.Reader) ([]byte, error) {
	var sha1s [][]byte
	for {
		dataBuf, err := ioutil.ReadAll(io.LimitReader(reader, defaultBlockSize))
		if err != nil {
			return nil, err
...
func is4MbParts(parts []int64) bool {
	for _, partSize := range parts[:len(parts)-1] {
		if partSize != defaultBlockSize {
			return false
		}
	}
	if parts[len(parts)-1] > defaultBlockSize {
		return false
	}
	return true
}
